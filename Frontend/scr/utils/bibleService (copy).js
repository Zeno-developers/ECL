...
class BibleService {
  constructor() {
    this.baseUrl = 'https://bible.helloao.org/api';
    this.versions = [
  ...
  // Get books - only call API for BSB, use local data for others
  async getBooks(version = 'BSB') {
    // Only BSB is available via API
    if (version !== 'BSB') {
    ...
    try {
      const data = await this.apiCall(`${this.baseUrl}/${version}/books.json`);
      return data.books;
    } catch (error) {
      console.warn(`Failed to fetch books for ${version}, using local data:`, error.message);
      return this.getLocalBooks(version);
    }
  }
  ...
  getLocalBooks(version) {
    return this.booksData.map(book => ({
      ...
      firstChapterApiLink: `${this.baseUrl}/${version}/${book.id}/1.json`,
      lastChapterApiLink: `${this.baseUrl}/${version}/${book.id}/${book.chapters}.json`,
  ...
  async getChapter(book, chapter, version = 'BSB') {
    ...
    try {
      const chapterData = await this.apiCall(
        `${this.baseUrl}/${version}/${book}/${chapter}.json`
      );
  ...
  // Get daily verse - only use BSB for real data
  async getDailyVerse() {
    const today = new Date().toDateString();
    const version = 'BSB'; // Always use BSB for daily verse since it's the only working version
    ...
    try {
      const verseData = await this.getVerse(selectedRef, version);
      return {
        reference: selectedRef,
        version: version,
        text: verseData.text,
        date: today,
        fullReference: verseData.fullReference,
        offline: verseData.offline || false
      };
    } catch (error) {
