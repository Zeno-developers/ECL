...
const convert24To12Hour = (time24) => {
  ...
  } catch { return { hour: '', minute: '', ampm: 'AM' } }
}
...
export default function EditEventPage() {
  ...
  const loadEvent = async () => {
    try {
      ...
      const time12 = convert24To12Hour(event.time)
      setFormData({
        title: event.title || '',
        date: event.date ? event.date.split('T')[0] : '',
        hour: time12.hour,
        minute: time12.minute,
        ampm: time12.ampm,
        type: event.type || 'service',
        description: event.description || '',
        location: event.location || 'A3313 Rd 3935, Mtubatuba, South Africa',
        is_published: Boolean(event.is_published ?? 1),
      })
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.message?.includes('Unauthorized')) { toast.error('Please log in'); navigate('/login') }
      else if (error.message?.includes('not found')) { toast.error('Event not found'); navigate('/events/manage') }
      else toast.error(error.message || 'Failed to update event')
    } finally {
      setSaving(false)
    }
  }
    ...
    <DashboardShell>
      <div className="mx-auto max-w-4xl px-5 sm:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center gap-4 border-b border-warm-charcoal/[0.07] pb-8">
          <button onClick={() => navigate('/events/manage')} disabled={saving} className="rounded-xl border border-warm-charcoal/[0.07] bg-white p-2.5 text-warm-muted transition hover:text-warm-charcoal shadow-sm">
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 className="text-3xl font-black tracking-tighter text-warm-charcoal">EDIT EVENT</h1>
            <p className="mt-1 text-sm text-warm-muted">Update the event details below.</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-5">
            <div className="rounded-2xl border border-warm-charcoal/[0.07] bg-white p-6 space-y-5 shadow-sm">
              <div>
                ...
                <input type="text" name="title" value={formData.title} onChange={handleChange} className={errors.title ? inputErrCls : inputCls} placeholder="Enter event title" disabled={saving} maxLength={100} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" name="date" value={formData.date} onChange={handleChange} className={errors.date ? inputErrCls : inputCls} disabled={saving} />
                  {errors.date && <p className="mt-1 text-[10px] text-red-600">{errors.date}</p>}
                </div>
                <div>
                  <label className={labelCls}>Event Time *</label>
                  <div className="grid grid-cols-3 gap-2">
                    <select name="hour" value={formData.hour} onChange={handleChange} className={errors.hour ? inputErrCls : inputCls} disabled={saving}>
                      <option value="">Hour</option>
                      {HOUR_OPTIONS.map((h) => <option key={h} value={h}>{h}</option>)}
                    </select>
                    <select name="minute" value={formData.minute} onChange={handleChange} className={errors.minute ? inputErrCls : inputCls} disabled={saving}>
                      <option value="">Min</option>
                      {MINUTE_OPTIONS.map((m) => <option key={m} value={m}>{m}</option>)}
                    </select>
                    <select name="ampm" value={formData.ampm} onChange={handleChange} className={inputCls} disabled={saving}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="type" value={formData.type} onChange={handleChange} className={errors.type ? inputErrCls : inputCls} disabled={saving}>
                    {EVENT_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelCls}>Location *</label>
                  <input type="text" name="location" value={formData.location} onChange={handleChange} className={errors.location ? inputErrCls : inputCls} placeholder="Enter event location" disabled={saving} maxLength={200} />
              ...
              <div>
                ...
                <textarea name="description" value={formData.description} onChange={handleChange} rows={4} className={`${errors.description ? inputErrCls : inputCls} resize-none`} placeholder="Enter event description (optional)" disabled={saving} maxLength={500} />
              ...
              <div className="rounded-xl border border-warm-gold/20 bg-warm-gold/[0.04] p-4">
                ...
                <p className="mt-1.5 ml-7 text-xs text-warm-muted">Turn this on to keep the event visible on the public Events page.</p>
              </div>
            </div>

            <div className="flex gap-3 justify-end">
              <button type="button" onClick={() => navigate('/events/manage')} disabled={saving} className="rounded-xl border border-warm-charcoal/[0.07] bg-white px-5 py-2.5 text-xs font-semibold text-warm-muted transition hover:text-warm-charcoal disabled:opacity-50">
                Cancel
              </button>
          ...
          <div className="rounded-2xl border border-warm-charcoal/[0.07] bg-white p-5 space-y-4 h-fit sticky top-6 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                ...
                ['Date & Time', formData.date ? `${new Date(formData.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}${formData.hour ? ` at ${formatTimePreview()}` : ''}` : '—'],
            ...
            <div className="rounded-xl border border-warm-charcoal/[0.07] bg-warm-ivory p-3 space-y-1.5">
              <p className="text-[9px] font-bold tracking-[0.2em] text-warm-gold/70">EDITING TIPS</p>
              {['Update attendees if event details change', 'Verify date and time accuracy', 'Save changes before leaving'].map((tip) => (
                <p key={tip} className="text-[10px] text-warm-muted">· {tip}</p>
              ))}
            </div>
          </div>
        </div>
