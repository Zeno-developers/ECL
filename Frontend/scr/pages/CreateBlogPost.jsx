...
import { ArrowLeft, Save, Eye, Tag, Image, Clock } from 'lucide-react'
...
export default function CreateBlogPost() {
  ...
  const handleSubmit = async (e, status = 'draft') => {
    ...
    try {
      ...
      await blogAPI.createPost(postData, user.token)
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-warm-charcoal/[0.07] pb-8">
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 text-sm text-warm-muted transition hover:text-warm-charcoal"
            >
              <ArrowLeft size={16} />
              Blog
            </button>
            <span className="text-warm-charcoal/20">|</span>
            <h1 className="text-3xl font-black tracking-tighter text-warm-charcoal">NEW POST</h1>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 rounded-xl border border-warm-charcoal/[0.07] bg-white px-4 py-2.5 text-xs font-semibold text-warm-muted transition hover:border-warm-charcoal/[0.12] hover:text-warm-charcoal shadow-sm disabled:opacity-50"
            >
              <Save size={13} />
              SAVE DRAFT
            </button>
        ...
        {preview ? (
          <div className="rounded-2xl border border-warm-charcoal/[0.07] bg-white p-8 shadow-sm">
            <div className="flex items-center gap-4 mb-6">
              <span className="rounded-full px-3 py-1 text-[9px] font-bold tracking-[0.18em] text-warm-gold bg-warm-gold/10 capitalize">
                {formData.category}
              </span>
              <span className="text-xs text-warm-muted">Preview</span>
            </div>
            {primaryImage && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className={panelCls}>
                ...
                <textarea name="content" value={formData.content} onChange={handleChange} placeholder="Write your post content here. Press Enter for a new line and leave a blank line for a new paragraph." rows={20} className={`${inputCls} resize-none font-mono text-sm`} required />
            ...
            <div className="space-y-5">
              ...
              <div className={panelCls}>
                ...
                <div className="flex gap-2 pt-1">
                  <button type="button" onClick={(e) => handleSubmit(e, 'draft')} disabled={loading} className="flex-1 rounded-xl border border-warm-charcoal/[0.07] bg-white py-2.5 text-xs font-semibold text-warm-muted transition hover:text-warm-charcoal disabled:opacity-50">
                    Save Draft
                  </button>
                  <button type="button" onClick={(e) => handleSubmit(e, 'published')} disabled={loading} className="flex-1 rounded-xl bg-warm-gold py-2.5 text-xs font-bold tracking-[0.12em] text-warm-espresso transition hover:opacity-90 disabled:opacity-50">
                    Publish
                  </button>
                </div>
              </div>
