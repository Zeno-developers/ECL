...
-- Zones table (hierarchical structure) - NO FOREIGN KEYS
...
CREATE TABLE IF NOT EXISTS cells (
    ...
    meeting_day ENUM('sunday','monday','tuesday','wednesday','thursday','friday','saturday') DEFAULT 'monday',
...
-- Add foreign keys after all tables are created (simplified - no self-referencing)
ALTER TABLE cells ADD FOREIGN KEY (zone_id) REFERENCES zones(id) ON DELETE SET NULL;
ALTER TABLE cells ADD FOREIGN KEY (cell_leader_id) REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE zones ADD FOREIGN KEY (zone_leader_id) REFERENCES users(id) ON DELETE SET NULL;
...
CREATE TABLE IF NOT EXISTS members (
    ...
    notes TEXT,
    spiritual_gifts LONGTEXT NULL,
    ministry_interests LONGTEXT NULL,
    is_active BOOLEAN DEFAULT 1,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
ALTER TABLE giving ADD FOREIGN KEY (visitor_id) REFERENCES visitors(id) ON DELETE SET NULL;
...
-- ============================================
-- CHURCH MANAGEMENT SYSTEM TABLES
-- ============================================

-- `zones` and `cells` are defined in the core section above.
-- Do not redefine them here; only add dependent church-management tables below.
...
-- Automated meeting polls for cells and zones
CREATE TABLE IF NOT EXISTS meeting_polls (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_meeting_polls_lookup (audience_type, audience_id, meeting_date),
    INDEX idx_meeting_polls_status (status),
    FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS meeting_poll_responses (
    ...
    INDEX idx_meeting_poll_responses_attendance (attendance_status),
...
-- Absence requests table (pre-marking)
CREATE TABLE IF NOT EXISTS absence_requests (
    ...
    INDEX idx_user_date (user_id, absence_date)
);

-- Cell change requests table
...
-- Engagement scores table (monthly)
...
CREATE TABLE IF NOT EXISTS follow_up_notes (
    ...
    FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE CASCADE,
    INDEX idx_follow_member (member_id),
...
SELECT '00000000-0000-0000-0000-000000000001', 'admin@elchurch.site', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'System', 'Administrator', 'admin', 1, 1
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'admin@elchurch.site');
...
SELECT 'Eternal Love Church', 'A3313 Rd', 'Mtubatuba', 'KwaZulu-Natal', 'South Africa', '0727641137', 'info@elchurch.site', 'https://elchurch.site'
WHERE NOT EXISTS (SELECT 1 FROM church_info LIMIT 1);
